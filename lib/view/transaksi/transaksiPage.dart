...
Future<void> _fetchFilmData() async {
  ...
  try {
    ...
    final transaksiResponse = await TransaksiReq.createTransaksi(newTransaksi);
  ...
  try {
    ...
    final updatedResponse = await TransaksiReq.updateTransaksi(_transaksi!.id, updatedTransaksi);
 ...
 void _navigateToDetailPage() async {
  if (_transaksi == null) {
    await _createInitialTransaksi(); // Pastikan transaksi dibuat terlebih dahulu.
  }
  ...
  Navigator.push(
    context,
    MaterialPageRoute(
      builder: (context) => TransaksiDetailPage(transaksiId: _transaksi!.id),
    ),
  );
}
